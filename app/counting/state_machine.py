...
class StateMachineConfig:
    ...
    # Hard business rule: this is a vehicle counter.
...
class CrossingStateMachine:
    ...
    def evaluate_candidate(self, row: pd.Series) -> dict:
        ...
        # BLOCKING RULE: person/non-vehicle candidates can be audited but
        # can NEVER reach COUNTED. This is enforced before any evidence
        # threshold so a person cannot be rescued by fast/short logic.
        vehicle_classes = {str(v).lower().strip() for v in self.config.vehicle_classes}
        if class_name not in vehicle_classes:
            history.append(CrossingState.REJECTED.value)
            reject_reasons.append(f"non_vehicle_class:{class_name}")
            return self._result(
                candidate_id,
                CrossingState.REJECTED,
                history,
                "vehicle_only_counter",
                score,
                False,
                False,
                "",
        ...
        # Fast crossing is a rescue path, not a free pass. It must have
        # explicit normal-motion evidence and a usable class signal.
        if fast_crossing:
            normal_displacement = abs(self._float(row, "normal_displacement_px", 0.0))
            if direction_conf < self.config.min_direction_confidence:
                review_reasons.append("fast_crossing_low_direction_confidence")
            if normal_displacement < 4.0:
                review_reasons.append("fast_crossing_weak_normal_motion")
        ...
        if review_reasons:
            ...
            can_rescue = (
                ...
                and score >= floor
                and not any(r.startswith("fast_crossing_") for r in review_reasons)
            )
            if can_rescue:
    ...
    def run(self, candidates: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Canonical counter-facing status MUST come from Phase 3. The input
        # candidate already contains pre-state-machine eligibility/counting
        # fields, so pandas may suffix the Phase 3 columns during merge.
        # Explicitly overwrite the canonical fields from the state-machine
        # result to prevent a rejected PERSON candidate from leaking through.
        if "count_eligibility_phase3" in enriched.columns:
            enriched["count_eligibility"] = enriched["count_eligibility_phase3"].fillna(False).astype(bool)
        else:
            enriched["count_eligibility"] = enriched["count_eligibility"].fillna(False).astype(bool)

        if "counted_phase3" in enriched.columns:
            enriched["counted"] = enriched["counted_phase3"].fillna(False).astype(bool)
        else:
